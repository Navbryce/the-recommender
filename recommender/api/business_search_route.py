...
from recommender.recommend.in_memory_recommendation_repository import InMemoryRecommendationRepository
...
recommendation_manager: RecommendationManager = RecommendationManager(recommender, InMemoryRecommendationRepository())
session_manager: SessionManager = SessionManager(recommendation_manager, InMemorySessionRepository())
...
def new_business_search() -> SessionCreationResponse:
    ...
    business_recommendation: DisplayableRecommendation = session_manager.get_first_recommendation(session_id=session.id)
...
def next_recommendation(session_id: str) -> DisplayableRecommendation:
    ...
    if recommendation_action_as_string not in RecommendationAction.__members__:
        raise ValueError(f"Unknown recommendation action {recommendation_action_as_string}")

    recommendation_action: RecommendationAction = RecommendationAction[recommendation_action_as_string]

    recommendation = session_manager.get_next_recommendation(session_id=session_id,
